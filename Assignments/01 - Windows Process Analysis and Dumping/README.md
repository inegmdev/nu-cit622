...
Check the `./Demo` folder you will have a pre-build executable, to run it use this command:
`$ ./procinfo -p <pid>` or `$ ./procinfo -n <name_of_exec>`, Examples:

```
$ ./procinfo -n msedge.exe
...
.... 
```
Note: For the full expected ouput please check [msedge_exe_procinfo.log](./__OUT/msedge_exe_procinfo.log)
You can check also the [notepad_exe_debugged_procinfo.log](./__OUT/notepad_exe_debugged_procinfo.log)

## Screenshots

![Procinfo in action](./__OUT/procinfo_inaction.png)
![Procinfo in action](./__OUT/procinfo_inaction2.png)
